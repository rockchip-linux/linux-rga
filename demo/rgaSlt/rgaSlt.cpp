...
#else
    memset(bo_src.ptr,0x55,4*1280*720);
#endif

#if 0
    //get_buf_from_file(bo_dst.ptr, dstFormat, dstWidth, dstHeight, 1);
#else
    memset(bo_dst.ptr,0x00,4*1280*720);
#endif
    ...
    while(1) {
        ...
        if (ret) {
            printf("rgaGetsrcFd fail : %s,hnd=%p \n",
                                            strerror(errno),(void*)(bo_src.handle));
        }
		/********** get dst_Fd **********/
		...
		unsigned int *pnow = (unsigned int *)bo_dst.ptr;
		int errCount = 0;
		int rightCount = 0;
